...
const calcNodeInsertPosition = (
  ...
  // 判断是否落在斜左上三角形区域内
  const isOverSecondDiagonal =
    relatY < -1 * (rectHeight / rectWidth) * relatX + rectHeight;
...
const OverlayDom = observer(
  (props: {
    ...
    console.log("attrs",node, node.attrs);
    ...
    const onDragOverHandler = (event: DragEvent<HTMLDivElement>) => {
      ...
      if (
        pageApi.draggingNode &&
        !!pageApi.draggingNode.findElementById(node.id)
      ) {
      ...
      if (!pageApi.draggingNode || pageApi.draggingNode.id == node.id) {
        return;
      }
    ...
    return (
      <>
        ...
        {Object.values(child).map((n) => (
          <OverlayDom node={n} zIndex={zIndex + 1} />
        ))}
      </>
    );
  }
...
const DetectOverlay = () => {
  ...
  return (
    ...
    >
      {pageApi.sort &&
        Object.values(pageApi.domTree).map((n) => (
          <OverlayDom key={n.id} node={n} />
        ))}
    </OverlayRoot>
  );
};
